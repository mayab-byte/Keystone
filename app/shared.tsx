...
/* Keystone teal palette:
   #123C3E (dark — dark sections & text-on-light) · #226D6D (primary accent) ·
   #61C5BD (bright — accent on dark) · #eef4f3 (light bg) · #ffffff.
   Legacy token names (teal/green) are kept but now carry teal values.
   --ks-teal-ink is the darkest teal, used for small text/links on light
   backgrounds so it clears the AA 4.5:1 contrast floor. The gradient runs
   bright→primary so it stays legible as headline text and thin accents on the
   dark teal sections, and reads with dark text on buttons. */
export const brand = {
  "--ks-teal": "#226D6D",
  "--ks-teal-ink": "#123C3E",
  "--ks-green": "#61C5BD",
  "--ks-purple": "#226D6D",
  "--ks-gray": "#a8c9c6",
  "--ks-grad": "linear-gradient(120deg, #61C5BD 0%, #226D6D 100%)",
} as CSSProperties;
...
export function KeystoneHeader() {
  return (
    <>
      ...
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:absolute focus:right-3 focus:top-3 focus:z-50 focus:rounded-full focus:bg-[#123C3E] focus:px-5 focus:py-2.5 focus:font-semibold focus:text-white"
      >
        דילוג לתוכן הראשי
      </a>
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#123C3E] text-white">
...
export function KeystoneFooter() {
  return (
    <>
      <footer className="border-t border-white/10 bg-[#123C3E] pb-10 pt-8 text-white">
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-[#123C3E]/90 px-4 pb-[max(env(safe-area-inset-bottom),0.75rem)] pt-3 backdrop-blur md:hidden">
